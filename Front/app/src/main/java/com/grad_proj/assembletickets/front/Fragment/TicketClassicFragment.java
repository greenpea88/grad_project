...
public class TicketClassicFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        classicShowAdapter.setOnItemClickListener(new ShowAdapter.OnItemClickListener() {
            @Override
            public void onItemClicked(View v, int position) {
                ...
                ((HomeActivity)getActivity()).replaceFragment(ShowDetailFragment.newInstance(classicShowAdapter.getItem(position)));
            }
        });

        classicTicketList.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                super.onScrolled(recyclerView, dx, dy);
                if(!classicTicketList.canScrollVertically(1)){
        ...
        classicShowRefresh = (SwipeRefreshLayout)view.findViewById(R.id.classicSwipeToRefresh);
        classicShowRefresh.setOnRefreshListener(this);
    ...
    private void setClassicList(List<Show> loadedShows){
        for(int i=0; i<loadedShows.size(); i++){
            Show show = loadedShows.get(i);

            //data를 adpater에 추가하기
            classicShowAdapter.addItem(show);
        }
        classicShowAdapter.notifyDataSetChanged();
    }
    ...
    private class GetClassicShows extends AsyncTask<String, Void ,List<Show>> {
...
//            super.onPreExecute();
            if(classicShowAdapter.getItemCount()!=0){
...
//            super.onPostExecute(shows);
    ...
    private class GetNewShows extends AsyncTask<String, Void ,List<Show>>{
        ...
        protected List<Show> doInBackground(String... strings) {
            ...
            String refresh = classicShowAdapter.getItem(0).getRegisteredTime();
...
//                System.out.println(newLoadedShows.size());
        ...
        protected void onPostExecute(List<Show> shows) {
            ...
            if(newLoadedShows.size()!=0){
                //현재 리스트 앞쪽으로 붙여야함
                classicShowAdapter.addNewItems(newLoadedShows);
                classicShowAdapter.notifyDataSetChanged();
            }
            else{
                Toast toast = Toast.makeText(view.getContext(),"더 이상 불러올 공연이 없습니다.",Toast.LENGTH_SHORT);
